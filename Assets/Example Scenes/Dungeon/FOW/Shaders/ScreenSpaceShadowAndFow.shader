...
Shader "Hidden/Internal-ScreenSpaceShadowsAndFow" {
    Properties{
 ...
 */
    ...
    */
    ...
    float SampleFOW(float3 wpos)
    {
        float2 uv = invlerp(_Bounds.xy, _Bounds.zw, wpos.xz);
        return  saturate(tex2D(_FOW, uv).r);
    }
    ...
    fixed4 frag_hard(v2f i) : SV_Target
    {
        ...
        fixed4 res = shadow * SampleFOW(wpos);
        return res;
    }
